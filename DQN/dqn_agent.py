...
class DQNagent:
    ...
    """
    def __init__(self, env, episodes=1000, t_max=1000, buffer_size=50000, batch_size=128, device='cpu'):
        self.env = env
        ...
        self.network_local = DenseDQN(self.state_space, self.action_space).to(self.device)     # local network
        self.network_target = DenseDQN(self.state_space, self.action_space).to(self.device)     # target network

        self.optimzer = torch.optim.RMSprop(self.network_local.parameters(), lr=0.0005)
    ...
    def learn(self, experiences, soft_update=False):
        """
        ...
        q_target_next = self.network_target(next_states).detach().max(axis=1)[0].unsqueeze(1)
    ...
    def run(self, save_every=100, render=False, project_name=None):
        ...
        wandb.init(project="DQN-Pytorch", name=f"{project_name}-DQN")
    ...
    def test(self, file, record=False):
        ...
        if record:
            wandb.init(project="DQN-Pytorch", name="Test Video-DQN", monitor_gym=True)
...
if __name__ == "__main__":
    ...
    agent = DQNagent(env, 2000)
    # agent.run() # uncomment to train model
    agent.test('checkpoints/CartPole-v1/dqn/CartPole-v1-hard_update.pth', record=False)
