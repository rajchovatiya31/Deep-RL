...
class DDQNagent:
    ...
    def learn(self, experiences, soft_update=False):
        ...
        # calculate target and expacted value
    ...
    def run(self, save_every=100, render=False, project_name=None):
        ...
        wandb.init(project="DQN-Pytorch", name=f"{project_name}-DDQN")
    ...
    def test(self, file, record=False):
        ...
        if record:
            wandb.init(project="DQN-Pytorch", name="Test Video-DDQN", monitor_gym=True)
...
if __name__ == "__main__":
    ...
    agent = DDQNagent(env, episodes=2000)
    # agent.run() # uncomment to train model
    agent.test('checkpoints/CartPole-v1/ddqn/CartPole-v1-pytorch-ddqn.pth', record=False)
