...
class DQNagent:
    def __init__(self, env, max_gradient_norm=None, episodes=1000, t_max=1000, buffer_size=50000, batch_size=128, device='cpu'):
        ...
        self.memory = PrioritizedReplay(self.buffer_size, self.batch_size)
    ...
    def learn(self, experiences):
        ...
        """
        idxs = experiences[0]
        weights = torch.from_numpy(experiences[1]).float()
        states = torch.from_numpy(experiences[2]).float()
        actions = torch.from_numpy(experiences[3]).long()
        rewards = torch.from_numpy(experiences[4]).float()
        next_states = torch.from_numpy(experiences[5]).float()
        dones = torch.from_numpy(experiences[6]).float()
        ...
        self.memory.update_priorities(priorities, idxs)

        self.soft_update(self.network_local, self.network_target, 0.1)
    ...
    def run(self, save_every=100, render=False, project_name=None):
        ...
        wandb.init(project="DQN-Pytorch", name=f"{project_name}-PER")
    ...
    def test(self, file, record=False):
        ...
        if record:
            wandb.init(project="DQN-Pytorch", name="Test Video-PER", monitor_gym=True)
...
if __name__ == "__main__":
    ...
    # agent.run(project_name='CartPole-v1-PER') # uncomment to train model
    agent.test('checkpoints/CartPole-v1/PER/CartPole-v1-pytorch-PER.pth', record=False)
